...
class DataHelper(object):
    ...
    def getXmlData(self, value):
        dom = xml.dom.minidom.parse('F:/pythonscirpt/AutoTest/Data-Driven/51cto.xml')
    ...
    def getXmlUser(self, parent, child):
        dom = xml.dom.minidom.parse('F:/pythonscirpt/AutoTest/Data-Driven/51cto.xml')
        # F:/pythonscirpt/AutoTest0103/Data-Driven/51cto.xml
        db = dom.documentElement
...
if __name__ == '__main__':
    ...
    print DataHelper().getXmlUser('login1', 'username')
    print DataHelper().getXmlUser('login1', 'passwd')


'''
import os, csv, xlrd, json
import xml.dom.minidom
import sqlite3
import MySQLdb
import config
...
class DataHelper(object):
    ...
    def readExcel(self, rowValue, colValue):
        ...
        # print type(sheet.cell_value(rowValue, colValue))
    ...
    def readExcels(self):
        ...
        # return json.dumps(rows, encoding='utf-8', ensure_ascii=False)
        #     rows = json.dumps(rows, encoding='utf-8', ensure_ascii=False)
        return rows

    def getXmlData(self, value):
        dom=xml.dom.minidom.parse(Config.data_dirs()+"/51cto.xml")
        db=dom.documentElement
        name=db.getElementsByTagName(value)
        nameValue=name[0]
        return nameValue.firstChild.data
...
if __name__=='__main__':
        ...
        print DataHelper().readExcel(1,0)
        # print type(DataHelper().readExcels())
'''